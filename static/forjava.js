...
var padding_x = 2;
var padding_y = 10;
var blocks_width = (300 / 20) - padding_x;
var offset_x =(300 - blocks_width*BLOCK_COLUMNS)/2;
var offset_y = 15;
var blocks_height = 5;
var blocks = new Array(BLOCK_ROWS);
for(var i=0; i< BLOCK_ROWS; i++) {
    blocks[i] = new Array(BLOCK_COLUMNS);
    for (var j = 0; j < BLOCK_COLUMNS; j++) {
        blocks[i][j] = Wrapper.create_rectangle({
            _group: 'blocks',
...
Wrapper.create_right_bound({'coord':300});
Wrapper.create_top_bound({'coord':0});
Wrapper.create_bottom_bound({'coord':150});
Wrapper.onaction('left',function(){
    platform.vx = -5;
});
Wrapper.onaction('right',function(){
    platform.vx = 5;
});
Wrapper.onaction('stop',function(){
    platform.vx = 0;
});
function send(message){
    Wrapper.actions.push(message);
}
function get_state(){
